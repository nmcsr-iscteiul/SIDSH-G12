...
echo '<table class="piereg_rw_main_table">';
	echo '<tr>';
		echo '<td valign="top">';
			...
			if( function_exists( 'wp_nonce_field' )) wp_nonce_field( 'piereg_wp_show_in_restrict_nonce','piereg_show_in_restrict_nonce'); ?>
            <div style="width:100%;display:inline-block;">
            	<fieldset  style="padding:5px 5px;">
                    <label><input <?php echo (($visibility_status == "After Login") ? 'checked="checked"' : ""); ?> type="checkbox" value="visibility_status=After Login" id="pr_ristrict_widget_after_login" /><?php _e("Show to Logged in Users","pie-register"); ?></label>
                    &nbsp;&nbsp;&nbsp;
                    <label><input <?php echo (($visibility_status == "Before Login") ? 'checked="checked"' : ""); ?> type="checkbox" value="visibility_status=Before Login" id="pr_ristrict_widget_before_login" /><?php _e("Show to Non-Logged in Users","pie-register"); ?></label>
                    &nbsp;&nbsp;&nbsp;
                    <label><input <?php echo (($visibility_status == "") ? 'checked="checked"' : ""); ?> type="checkbox" value="visibility_status=" id="pr_ristrict_widget_off" /><?php _e("Show All Users","pie-register"); ?></label>                    
			...
			echo '<table class="widefat piereg_user_roles" style="width:100%;">';
				...
				echo '<span><a id="pieregister_rw_select_all" title="Select All User Roles">'.(__("Select All","pie-register")).'</a></span>';
				echo "&nbsp;/&nbsp;";
				echo '<span><a id="pieregister_rw_unselect_all" title="Unselect All User Roles">'.(__("Unselect All","pie-register")).'</a></span>';
				echo '</div>';
			echo '</div>';
			echo '<p class="submit" style="clear:both;margin-top:25px;">';
				echo '<input type="submit" value="'.(__("Save Changes","pie-register")).'" />';
			echo '</p>';
			echo '</form>';
		echo '</td>';
	echo '</tr>';
