	...
	class PieRegisterDashWidgetStats{
		...
		function register_widget() {
			...
			if ( current_user_can('manage_options') )
				wp_register_sidebar_widget( 'piereg_form_stats', __(' Pie Register Stats','pie-register'), array($this, 'widget'), array( 'settings' => 'options-general.php?page=pie-register' ) );			
		}
      ...
      <div class="piereg_stats_row" <?php echo (($pr_stats_key == "login" || $pr_stats_key == "forgot")?'style="display:none"':''); ?> >
        <?php
                if($pr_stats_key == "login"){
                    echo '<div class="piereg_stats_title"><h1>'.(__("Login","pie-register")).'</h1></div>';
                }
                elseif($pr_stats_key == "forgot"){
                    echo '<div class="piereg_stats_title"><h1>'.(__("Forgot Password","pie-register")).'</h1></div>';
                }
                elseif($pr_stats_key == "register"){
                    echo '<div class="piereg_stats_title"><h1>'.(__("Registration","pie-register")).'</h1></div>';
                }
        ...
        <div class="piereg_stats_col">
          ...
          <div class="piereg_stats_label"> <span>
            <?php _e("Conversions","pie-register"); ?>
            </span> </div>
        </div>
        <?php
                } ?>
