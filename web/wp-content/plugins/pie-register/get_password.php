...
$warning 	= apply_filters('piereg_reset_password_warning',__("Enter your new password below.",'pie-register')); # newlyAddedHookFilter
$success	= "";
$errors 	= new WP_Error();
...
if( isset($_POST['piereg_get_password_nonce']) && wp_verify_nonce($_POST['piereg_get_password_nonce'], 'piereg_wp_get_password_nonce') )
{
	...
	if ( ( ! $errors->get_error_code() ) && isset( $this->pie_post_array['pass1'] ) && !empty( $this->pie_post_array['pass1'] ) ) {
		...
		$success = apply_filters('piereg_reset_password_success',__( 'Your password has been reset.','pie-register' )); # newlyAddedHookFilter	
	}
