...
if(!isset($_GET['pie_id']) || empty($wp_registered_widgets[$_GET['pie_id']]))	
	...
	$url = $this->pr_rw_admin_url . '&amp;act=edit&amp;pie_id=' . esc_attr($_GET['pie_id']);
	$esc_pie_id = esc_attr($_GET['pie_id']);	
	$widget = &$wp_registered_widgets[$esc_pie_id];
	if(!empty($_GET['option'])){
	
		echo '<div class="pr_rw_breadcrums"><a href="', esc_url($this->pr_rw_admin_url) ,'">'.(__("Custom widgets","pie-register")).'</a> <span class="piereg_breadcrum_bulet">&nbsp;&nbsp;&nbsp;</span> '.(__("Customize","pie-register")).' "'.$widget['name'].'"</div>';	
